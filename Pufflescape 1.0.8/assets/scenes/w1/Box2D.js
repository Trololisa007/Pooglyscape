...
const VELOCITY_FLIP_AMOUNT = 1;
var ball;
var cValue;
var rectangle1;
const scale = .82;
var TORQUE_PER_UPDATE = 70;
var maxVelocity = 20 * scale;
var maxAngularVelocity = 16
var ctrlPair;
var door;
var isActivelyRolling = false;
/* START OF COMPILED CODE */

class Box2D extends Phaser.Scene {
	...
	editorCreate() {
		...
		pauseScreen.scaleX = 1;
		pauseScreen.scaleY = 1;
	...
	create() {
		console.log(this.game.plugins.get("Box2D"))
		box2d.ENABLE_ASSERTS = true
		box2d.b2World
		this.matter.world.runner.deltaMax = 1000/60;
		this.anims.resumeAll();
		this.sound.setVolume(1);
		const col = this.cache.json.get('collision9');
		...
		this.getLinearVelocity = function(body) {
			return Math.sqrt(Math.pow(body.velocity.x, 2) + Math.pow(body.velocity.y, 2));
		}
		...
		var rightDown = this.input.keyboard.addKey('RIGHT')
		var leftDown = this.input.keyboard.addKey('LEFT')
		this.rightDown = rightDown
		this.leftDown = leftDown
		// this.input.keyboard.on('keydown-' + 'LEFT', () => {
			// 	if(ball.body.angularVelocity > -maxAngularVelocity)
			// 	ball.body.torque += -TORQUE_PER_UPDATE;
			// });
	...
	update() {
			...
			if(this.rightDown.isDown && ball.body.angularVelocity < maxAngularVelocity) {
				ball.body.torque += TORQUE_PER_UPDATE;
				console.warn(ball.body.torque)
				this.scaleX = 1;
				this.puffleAnims.setFlip(false)
			}
		// var rightDown = this.input.keyboard.addKey('RIGHT', false, true).on('down', (event) => {
		// 	if(ball.body.angularVelocity < maxAngularVelocity) {
		// 		ball.body.torque += TORQUE_PER_UPDATE;
		// 		this.scaleX = 1;
		// 		this.puffleAnims.setFlip(false)
		...
		// });
			...
			if(this.leftDown.isDown && ball.body.angularVelocity > -maxAngularVelocity) {
				ball.body.torque += -TORQUE_PER_UPDATE;
				this.scaleX = -1
		...
		// var leftDown = this.input.keyboard.addKey('LEFT').isDown;
		// var rightDown = this.input.keyboard.addKey('RIGHT').isDown;
		// if (isNaN(ball.body.anglePrev)){
		// 	ball.body.angle = 0;
		// 	ball.body.anglePrev = 0;
		// }

		// if (leftDown || rightDown) {
		// 	isActivelyRolling = true;
		// 	if (rightDown && ball.body.angularVelocity < maxAngularVelocity) {
		// 		this.matter.body.setAngularVelocity(ball.body, ball.body.angularVelocity)
		// 		this.matter.body.setAngularVelocity(ball.body, ((ball.angle - ball.body.anglePrev) * ball.body.frictionAir) + ((ball.body.torque + TORQUE_PER_UPDATE) / ball.body.inertia) * Math.pow(1000/60, 2));
		// 		console.warn(ball.body.angularVelocity)
		// 	}
		// }
		// if (!(this.rightDown.isDown || this.leftDown.isDown) && (ball.body.angularVelocity > maxAngularVelocity || ball.body.angularVelocity < -maxAngularVelocity || null)) {
		// 	ball.setAngularVelocity(ball.body.angularVelocity / Math.abs(ball.body.angularVelocity) * maxAngularVelocity)
		// }



		if((this.rightDown.isDown || this.leftDown.isDown)) {

			//this.isActivelyRolling = false;
			let stableForce = Math.max(.5,Math.min(1,Math.min(Math.abs(ball.body.angularVelocity),Math.abs(this.getLinearVelocity(ball.body)))));
			console.log(stableForce)
			this.matter.setAngularVelocity(ball.body, ball.body.angularVelocity * stableForce);
		}
